Fig5D_plot_2 <- function(target_phenotype, target_rs, target_position, threshold){
  ...
  Fig6E_CL <- df2  %>% unite("ID_position", c(ID, snp)) %>% filter(max_abs_dMOCCS2score > threshold) %>%
  ...
  Fig6E_TF <- df2  %>% unite("ID_position", c(ID, snp)) %>% filter(max_abs_dMOCCS2score > threshold) %>%
