FigS1D_plot <- function(annotation_path){
  ...
  df_soft2  <- df_all %>% filter(Antigen != "Epitope tags"& Antigen != "GFP") %>% group_by(Antigen) %>% summarise(n_sample = n()) %>% arrange(desc(n_sample)) 
  TF_selected_list2 <- df_all2[1:20,]$Antigen %>% as.character()
  ...
  TF_selected_list <- unique(c(TF_selected_list1, TF_selected_list2, TF_selected_list3))
  df_plot3 <- c()
