Fig6C_plot <- function(target_phenotype, annotation_path, threshold){
  ...
  df2 <- df_phenotype_binded_all_selected_annotated2 %>% mutate(abs_dMOCCS2score = abs(dMOCCS2score)) %>% group_by(ID, snp, Antigen, Cell_type_class, Cell_type) %>% summarise(max_abs_dMOCCS2score = max(abs_dMOCCS2score))
  ...
  Fig6C_CL <- df2  %>% unite("ID_position", c(ID, snp)) %>% filter(max_abs_dMOCCS2score > threshold) %>%
  ...
  Fig6C_TF <- df2  %>% unite("ID_position", c(ID, snp)) %>% filter(max_abs_dMOCCS2score > threshold) %>%
