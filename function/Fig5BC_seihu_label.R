make_label <- function(figure){
  ...
  if(figure == "C"){
    ...
    snp_for_join <- df_phenotype_binded_all_selected_annotated_sei %>% unite("snp_for_join", c(position, alt_bp)) %>% .$snp_for_join %>% as.character()
    df_phenotype_binded_all_selected_annotated_sei2 <- df_phenotype_binded_all_selected_annotated_sei %>% select(-snp_for_join) %>% mutate(snp_for_join = snp_for_join)
    df2 <- df_phenotype_binded_all_selected_annotated_sei2 %>% mutate(abs_dMOCCS2score = abs(dMOCCS2score)) %>% group_by(ID, snp_for_join, Antigen, Cell_type_class, Cell_type) %>% summarise(max_abs_dMOCCS2score = max(abs_dMOCCS2score))
    ...
    p1_sei <- df3  %>% unite("ID_snp_for_join", c(ID, snp_for_join)) %>% filter(max_abs_dMOCCS2score > threshold) %>%
    ...
    #saveRDS(x_lab_ID, paste0("/Users/saeko/MOCCS_paper_public/data/Fig6/label/", target_phenotype, "_label_TF_sei.rds"))
    x_lab_ID <- readRDS("/Users/saeko/MOCCS_paper_public/data/Fig6/label/SLE_label_TF_rs_sei_added.rds")
    x_lab_ID2 <- x_lab_ID %>% .$SNP_rs %>% as.character() %>% rev()
    
    
    p2_sei <- df3  %>% unite("ID_snp_for_join", c(ID, snp_for_join)) %>% filter(max_abs_dMOCCS2score > threshold) %>%
      ggplot(aes(x = reorder(ID_snp_for_join, max_abs_dMOCCS2score), y = max_abs_dMOCCS2score, fill = Cell_type_class))+
      geom_col() +
      theme(plot.title = element_text(face="bold",hjust = 0.5), 
            panel.grid.major = element_line(colour = "gray"),
    ...
    # 6C ref k-mer label ----
    df4 <- df_phenotype_binded_all_selected_annotated_sei2 %>% 
    ...
    p3_sei <- df4  %>% unite("ID_snp_for_join", c(ID, snp_for_join)) %>% filter(max_abs_dMOCCS2score > threshold) %>%
      ggplot(aes(x = reorder(ID_snp_for_join, max_abs_dMOCCS2score), y = max_abs_dMOCCS2score, fill = Cell_type_class))+
      geom_col() +
      theme(plot.title = element_text(face="bold",hjust = 0.5), 
            panel.grid.major = element_line(colour = "gray"),
    ...
    p4_sei <- df4  %>% unite("ID_snp_for_join", c(ID, snp_for_join)) %>% filter(max_abs_dMOCCS2score > threshold) %>%
      ggplot(aes(x = reorder(ID_snp_for_join, max_abs_dMOCCS2score), y = max_abs_dMOCCS2score, fill = Cell_type_class))+
      geom_col() +
      theme(plot.title = element_text(face="bold",hjust = 0.5), 
            panel.grid.major = element_line(colour = "gray"),
    ...
    snp_for_join <- df_phenotype_binded_all_selected_annotated_minus %>% unite("snp_for_join", c(position, alt_bp)) %>% .$snp_for_join %>% as.character()
    df_phenotype_binded_all_selected_annotated_minus2 <- df_phenotype_binded_all_selected_annotated_minus %>% select(-snp_for_join) %>% mutate(snp_for_join = snp_for_join)
    df2 <- df_phenotype_binded_all_selected_annotated_minus2 %>% mutate(abs_dMOCCS2score = abs(dMOCCS2score)) %>% group_by(ID, snp_for_join, Antigen, Cell_type_class, Cell_type) %>% summarise(max_abs_dMOCCS2score = max(abs_dMOCCS2score))
    df3 <- df2 %>% left_join(GWAS_df2, by = "snp_for_join") 
    ...
    x_lab_ID <- df3  %>% unite("ID_snp_for_join", c(ID, snp_for_join)) %>% filter(max_abs_dMOCCS2score > threshold) %>% arrange(desc(max_abs_dMOCCS2score)) %>% select(ID_snp_for_join)
    x_lab_ID2 <- x_lab_ID %>% separate(ID_snp_for_join, into = c("ID", "chr", "position", "alt_bp"), sep = "_") %>% .$ID %>% as.character() %>% rev()
    p1_minus <- df3  %>% unite("ID_snp_for_join", c(ID, snp_for_join)) %>% filter(max_abs_dMOCCS2score > threshold) %>%
      ggplot(aes(x = reorder(ID_snp_for_join, max_abs_dMOCCS2score), y = max_abs_dMOCCS2score, fill = Cell_type_class))+
      geom_col() +
      theme(plot.title = element_text(face="bold",hjust = 0.5), 
            panel.grid.major = element_line(colour = "gray"),
    ...
    p2_minus <- df3  %>% unite("ID_snp_for_join", c(ID, snp_for_join)) %>% filter(max_abs_dMOCCS2score > threshold) %>%
      ggplot(aes(x = reorder(ID_snp_for_join, max_abs_dMOCCS2score), y = max_abs_dMOCCS2score, fill = Cell_type_class))+
      geom_col() +
      theme(plot.title = element_text(face="bold",hjust = 0.5), 
            panel.grid.major = element_line(colour = "gray"),
    ...
    # 6C ref k-mer label ----
    df4 <- df_phenotype_binded_all_selected_annotated_minus2 %>% 
      mutate(abs_dMOCCS2score = abs(dMOCCS2score))%>% 
      select(ID, snp_for_join, Antigen, Cell_type_class, Cell_type, kmer_before_tou, kmer_after_tou, 
             dMOCCS2score, q_value, abs_dMOCCS2score) %>%
      group_by(ID, snp_for_join, Antigen, Cell_type_class, Cell_type) %>% 
    ...
    x_lab_ID <- df4  %>% 
      ...
      select(ref_kmer) 
    x_lab_ID2 <- x_lab_ID %>% .$ref_kmer %>% as.character() %>% rev()
    p3_minus <- df4  %>% unite("ID_snp_for_join", c(ID, snp_for_join)) %>% filter(max_abs_dMOCCS2score > threshold) %>%
      ggplot(aes(x = reorder(ID_snp_for_join, max_abs_dMOCCS2score), y = max_abs_dMOCCS2score, fill = Cell_type_class))+
      geom_col() +
      theme(plot.title = element_text(face="bold",hjust = 0.5), 
            panel.grid.major = element_line(colour = "gray"),
    ...
    x_lab_ID <- df4  %>% 
      ...
      select(alt_kmer) 
    x_lab_ID2 <- x_lab_ID %>% .$alt_kmer %>% as.character() %>% rev()
    p4_minus <- df4  %>% unite("ID_snp_for_join", c(ID, snp_for_join)) %>% filter(max_abs_dMOCCS2score > threshold) %>%
      ggplot(aes(x = reorder(ID_snp_for_join, max_abs_dMOCCS2score), y = max_abs_dMOCCS2score, fill = Cell_type_class))+
      geom_col() +
      theme(plot.title = element_text(face="bold",hjust = 0.5), 
            panel.grid.major = element_line(colour = "gray"),
    ...
    #x_lab_ID <- df3  %>% #unite("ID_snp_for_join", c(ID, snp_for_join)) %>% 
      #filter(max_abs_dMOCCS2score > threshold) %>% arrange(desc(max_abs_dMOCCS2score)) %>% select(SNP_rs)
    #saveRDS(x_lab_ID, paste0("/Users/saeko/MOCCS_paper_public/data/Fig6/label/", target_phenotype, "_label_TF_sei.rds"))
    x_lab_ID <- readRDS("/Users/saeko/MOCCS_paper_public/data/Fig6/label/CD_label_TF_rs_sei_added.rds")
    x_lab_ID2 <- x_lab_ID %>% .$SNP_rs %>% as.character() %>% rev()
  ...
  return(list(p1_sei, p2_sei, p3_sei, p4_sei, p1_minus, p2_minus, p3_minus, p4_minus))
}