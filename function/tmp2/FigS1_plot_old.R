FigS1_plot <- function(simulation_path){
  ...
  p7 <- df_significant2 %>% filter(N == 12000 & sigma == "W5" & q == 0.05) %>%
  ...
  p8 <- df_significant2 %>% filter(a == "[0.1,0.2]" & sigma == "W5" & q == 0.05) %>%
  ...
  # sigma
  df_significant2 <- transform(df_significant2, sigma= factor(sigma, levels = c("W5", "W2")))
  p9 <- df_significant2 %>% filter(a == "[0.1,0.2]" & N == 12000 & q == 0.05) %>%
