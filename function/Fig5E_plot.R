Fig5E_plot <- function(target_TF, path){
  
  library(tidyverse)
  if(target_TF != "CTCF"){
    df_all <- readRDS(paste0(path, "ADASTRA/dMOCCS2score_output_all/ADASTRA_dMOCCS2score_", target_TF, "_all.rds"))
  ...
  # bar plot
  if(file.exists("/Users/saeko/Documents/MOCCS/paper_figure/MOCCS-DB_paper/plot/Fig5/Fig5E_bar_plot.pdf") == FALSE){
    
    bar_df <- read_csv(paste0(path, "ADASTRA/Fig5E_barplot.csv"))
