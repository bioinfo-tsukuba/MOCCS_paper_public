Fig6D_plot <- function(target_phenotype, target_rs, target_position, annotation_path){
  ...
  Fig6D_CL <- df2  %>% unite("ID_position", c(ID, position)) %>% filter(max_abs_dMOCCS2score > 120) %>%
  ...
  Fig6D_TF <- df2  %>% unite("ID_position", c(ID, position)) %>% filter(max_abs_dMOCCS2score > 120) %>%
